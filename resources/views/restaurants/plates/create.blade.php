...
@section('content')
    <div class="container plates_create_page">
        ...
        <form action="{{ route('restaurants.plates.store') }}" class="d-flex flex-column" method="POST"
            enctype="multipart/form-data">
            @csrf
            <div class="mb-4">
                <label for="name">Nome Piatto *</label>
                <input class="form-control" type="text" name="name" id="name" value="{{ old('title') }}" required
            ...
            <div class="mb-4">
                ...
                <textarea class="form-control" type="text" name="ingredients" id="ingredients" required
                    maxlength="1000">{{ old('ingredients') }}</textarea>
                @error('ingredients')
                    <div class="text-danger fw-bold">{{ $message }}</div>
                @enderror
            </div>
            ...
            <input type="submit" value="Salva nuovo piatto" class="mt-4 w-25 text-center btn btn-success">
        </form>
